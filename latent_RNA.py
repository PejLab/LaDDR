...
def load_inputs(infile: Path, regions, parse_ids: bool) -> pd.DataFrame:
    """Load coverage files into one dataframe and normalize coverage"""
    ...
    counts = np.zeros((regions.shape[0], infiles.shape[0]), dtype=int)
    for i, fname in tqdm(enumerate(infiles['path']), total=len(infiles['path']), desc="Loading files"):
        d = pd.read_csv(fname, header=None, dtype=int, names=['count'], index_col=False)
        assert d.shape[0] == regions.shape[0]
        counts[:, i] = d['count']
...
def gene_fit_transform(df: pd.DataFrame, regions: pd.DataFrame, var_expl: float, n_pcs_max: int) -> PCA:
...
def fit(df: pd.DataFrame, regions: pd.DataFrame, var_expl_max: float, n_pcs_max: int) -> dict:
    """Fit PCA models for all genes"""
    models = {'var_expl_max': var_expl_max, 'n_pcs_max': n_pcs_max, 'models': {}}
    regions = regions.groupby('gene_id')
    n_genes = df.index.get_level_values('gene_id').nunique()
    for gene_id, x in tqdm(df.groupby('gene_id'), total=n_genes, desc="Fitting models"):
...
def transform(df: pd.DataFrame, regions: pd.DataFrame, models: dict) -> pd.DataFrame:
    """Apply PCA transformation to all genes"""
    regions = regions.groupby('gene_id')
    out = []
    n_genes = df.index.get_level_values('gene_id').nunique()
    for gene_id, x in tqdm(df.groupby('gene_id'), total=n_genes, desc="Applying transformation"):
...
def fit_transform(df: pd.DataFrame, regions: pd.DataFrame, var_expl_max: float, n_pcs_max: int) -> pd.DataFrame:
    """Fit and transform each gene one by one so not all models are stored in memory at once"""
    regions = regions.groupby('gene_id')
    out = []
    n_genes = df.index.get_level_values('gene_id').nunique()
    for gene_id, x in tqdm(df.groupby('gene_id'), total=n_genes, desc="Fitting and transforming"):
        reg = regions.get_group(gene_id)
...
def create_parser():
    ...
    parser_fit.add_argument('-i', '--inputs', type=Path, metavar='FILE', required=True, help='File containing paths to all input coverage files.')
    ...
    parser_transform.add_argument('-i', '--inputs', type=Path, metavar='FILE', required=True, help='File containing paths to all input coverage files. Base file name before first "." is sample ID.')
    ...
    parser_fit_transform.add_argument('-i', '--inputs', type=Path, metavar='FILE', required=True, help='File containing paths to all input coverage files. Base file name before first "." is sample ID.')
...
if __name__ == '__main__':
    ...
    if args.subcommand == 'fit':
        ...
        models = fit(inputs, regions, args.var_expl_max, args.n_pcs_max)
        ...
        output = transform(inputs, regions, models)
        ...
        output = fit_transform(inputs, regions, args.var_expl_max, args.n_pcs_max)
        output.to_csv(args.output, sep='\t', index=False, float_format='%g')
        print(f'PCA-transformed features saved to {args.output}', flush=True)
