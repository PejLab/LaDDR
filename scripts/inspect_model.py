...
assert not model.fpca

# Combine the mean/std from model.features with loadings from the PCA model (model.model)
...
phenos = pd.read_csv(args.phenotypes, sep='\t', index_col=0)
